...
import * as SubjectSearchAPI from '../../../services/api/studentSubjectSearch'
import * as SubjectSearchAPINonAuthenticated from '../../../services/api/nonAuthenticatedSubjectSearch'
...
import SubjectSearchResultsNonAuthenticated from '../../../atomic/organisms/nonAuthenticated/SubjectSearchResults'
...
class StudentSubjectSearchContainer extends React.Component {
    ...
    this.setState(
      ...
      async () => {
        ...
        if (this.props.isNoAuthenticated) {
          results = await SubjectSearchAPINonAuthenticated.getAll(
            this.processFilters(filters),
            page
          )
        } else {
          results = await SubjectSearchAPI.getAll(
            this.processFilters(filters),
            page
          )
        }
    ...
    const resultsComponent = (
      ...
      >
        {({ paginationComponent }) => (
            ...
            ( 
              <SubjectSearchResultsNonAuthenticated
                results={results}
...
export default withI18n()(connect(mapStateToProps, mapDispatchToProps)(StudentSubjectSearchContainer))
